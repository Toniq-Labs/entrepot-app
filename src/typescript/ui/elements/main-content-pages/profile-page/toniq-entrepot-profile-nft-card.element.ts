...
export const EntrepotProfileCardElement = defineToniqElement<{
    nft: Pick<
        UserNft & BaseFullProfileEntry,
        'collectionId' | 'nftId' | 'nftIndex' | 'listPrice' | 'nftNri'
    >;
}>()({
    ...
    renderCallback: ({inputs}) => {
        const hasListPrice = inputs.nft.listPrice > 0;
        const listPriceDisplay = hasListPrice ? toIcp(BigInt(inputs.nft.listPrice)) : 'Unlisted';
