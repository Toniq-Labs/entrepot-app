...
export default function NFT(props) {
    ...
    const getButtons = () => {
        ...
        if (props.nft.listing) {
            buttons.push([
                ...
                () => props.listNft(props.nft, buttonLoader),
            ]);
            ...
            if (wrappedCanisters.concat(unwrappedCanisters).indexOf(props.nft.canister) < 0) {
                ...
                if (unwrappedCanisters.indexOf(props.nft.canister) >= 0) {
                    buttons.push([
                        currentBtn == 0 && currentBtnText ? buttonLoadingText : 'Sell',
                        () => props.wrapAndlistNft(props.nft, buttonLoader),
                    ]);
                    buttons.push(['Transfer', () => props.transferNft(props.nft, buttonLoader)]);
                } else {
                    buttons.push([
    ...
    const mintNumber = () => {
        return EntrepotNFTMintNumber(props.collection, props.nft.index);
    };
    const nftImg = () => {
        return EntrepotNFTImage(props.collection, props.nft.index, props.nft.id);
    };
    const nftLink = () => {
        return EntrepotNFTLink(props.collection, props.nft.index, props.nft.id);
    };
    const nriLink = () => {
            ...
            return (
                <Grid item md={6} sm={6} xs={6}>
                    <Typography
                        ...
                        gutterBottom
                    >
                        {props.nft.metadata[0] === 0
                            ? 'Pack'
                            : '#' + props.nft.metadata[0] + ' - ' + t[props.nft.metadata[1]]}
                    </Typography>
                </Grid>
            );
        }
