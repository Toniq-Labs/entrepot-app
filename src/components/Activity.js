...
export default function Activity(props) {
  ...
  const refresh = async c => {
    ...
    var txs = await fetch(
      'https://us-central1-entrepot-api.cloudfunctions.net/api/canister/' + c + '/transactions',
    ).then(r => r.json());
                    ...
                    {transactions.length === 0 ? (
                      <div style={styles.empty}>
                        <Typography
                          ...
                          align="center"
                        >
                          There are currently no activity for this collection
                        </Typography>
                      </div>
                    ) : (
                      <>
                            ...
                            <Table style={{width: '100%', overflow: 'hidden'}}>
                              ...
                              <TableBody>
                                {transactions
                                  ...
                                  .filter(
                                    (token, i) => i >= (page - 1) * perPage && i < page * perPage,
                                  )
                                  .map((transaction, i) => {
                                    return (
                                      <Sold
