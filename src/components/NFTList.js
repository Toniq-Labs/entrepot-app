...
export default function NFTList(props) {
    ...
    const [collection, setCollection] = React.useState(
        props.collections.find((e) => e.route === params?.route),
    );
    ...
    const unwrapNft = async (token) => {
        ...
        var r = await extjs
            ...
            .canister(canister)
            .unwrap(token.id, [extjs.toSubaccount(props.currentAccount ?? 0)]);
        if (!r) {
            props.loader(false);
    ...
    const list = async (id, price, loader) => {
        ...
        api.token(id)
            .list(props.currentAccount, price)
            .then((r) => {
                if (r) {
    ...
    return (
        <div style={{minHeight: 'calc(100vh - 221px)'}}>
            <div style={styles.empty}>
                <h1>
                    My NFTs: <span style={{color: '#00d092'}}>{collection?.name}</span>
                </h1>
                <Button
                    variant={'outlined'}
                    color="primary"
                            ...
                            {nfts.length === 0 ? (
                                <div style={styles.empty}>
                                    <Typography
                                        ...
                                        align="center"
                                    >
                                        You own no NFTs in this collection!
                                    </Typography>
                                </div>
                            ) : (
                                <>
                                                        ...
                                                        case 'type':
                                                            if (
                                                                ...
                                                                if (
                                                                    ...
                                                                    return 0;
                                                                else if (a.metadata[0] === 0)
                                                                    return -1;
                                                                else if (b.metadata[0] === 0)
                                                                    return 1;
                                                                else if (
