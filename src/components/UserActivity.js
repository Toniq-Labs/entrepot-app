...
export default function UserActivity(props) {
  ...
  const [
    address,
    setAddress,
  ] = React.useState(params?.address ?? props.account.address ?? false);
  const [
    sort,
    setSort,
  ...
  const refresh = async clearcache => {
    ...
    setTokenCanisters(
      data.map(event => event.canister).filter(c => typeof getCollection(c) != 'undefined'),
    );
    setResults(data);
  };
  const styles = {
              ...
              <>
                {hideCollectionFilter ? (
                  ...
                  <>
                    ...
                    {tokenCanisters
                      .filter((a, i) => tokenCanisters.indexOf(a) == i) //filter unique
                      .map(canister => {
