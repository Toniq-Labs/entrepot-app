...
export default function ICSnakes(props) {
  ...
  const _updates = async () => {
    var stats = await api.canister("3ifmd-wqaaa-aaaah-qckda-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      if (qty === 1) {
        props.loader(true, "Reserving ICSnake...");
      } else {
        props.loader(true, "Reserving ICSnakes..");
      }
      const api = extjs.connect("https://boundary.ic0.app/", props.identity);
      var r = await api
        .canister("3ifmd-wqaaa-aaaah-qckda-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("3ifmd-wqaaa-aaaah-qckda-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "top", backgroundSize: "cover",backgroundImage:"url('/collections/icsnakes/banner.jpg')"}}></div>
        <h1>Welcome to the official ICSnakes sale</h1>
        </div>
        <Grid  justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
          ...
          <Grid className={classes.stat} item md={4} xs={6}>
            ...
            <span style={{fontWeight:"bold",color:"#00b894",fontSize:"2em"}}>{remaining !== false ? 9000-remaining : "Loading..."}</span>
          </Grid>
        </Grid>
        <br /><br />
        {remaining === false ? 
        ...
        : 
          <>{(!saleOver && remaining > 0) ?
            <>
              {whitelist ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>You are on the whitelist! You can mint 1 NFT at 0.45ICP for the first 24 hours only!</span></strong></p>
                </>:""
              }
              {Date.now() >= startTime ? 
                <>
                  <Grid justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
                    {whitelist ? 
                      ...
                      >
                        Buy 1 ICSnake<br />for {_showListingPrice(presaleprice)} ICP
                      </Button>
                    : buyOptions.map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        <Button
                          ...
                          style={{ fontWeight: "bold", margin: "0 auto" }}
                        >
                          Buy {o[0]} ICSnake{o[0] === 1 ? "" : "s"}<br />for {_showListingPrice(o[1])} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
          ...
          : 
            <p><strong><span style={{fontSize:"20px",color:"red"}}>Sorry, the sale is now over! You can grab your ICSnake from the marketplace soon!</span></strong></p>
          }</>
        }
      </div>
