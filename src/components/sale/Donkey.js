...
export default function Yolo(props) {
  ...
  const [price, setPrice] = React.useState(59000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1644937200000);
  const [whitelist, setWhitelist] = React.useState(false);
  var whitelistend = 1644939000000;
  var presaleprice = 30000000n;
  var totalToSell = 9000;
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("qjwjm-eaaaa-aaaah-qctga-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      ...
      var r = await api
        .canister("qjwjm-eaaaa-aaaah-qctga-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("qjwjm-eaaaa-aaaah-qctga-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "top", backgroundSize: "cover",backgroundImage:"url('/collections/donkey/banner.jpg')"}}></div>
        <h1>Welcome to the official DFINITY Donkey sale</h1>
        </div>
          ...
          <>{(!saleOver && remaining > 0) ?
            <>
              ...
              {whitelist && Date.now() >= startTime && Date.now() < whitelistend ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>You are on the whitelist! You can mint as many NFTs at 0.3ICP for the first 30 minutes only (limited to first 500 sold)!</span></strong></p>
