...
export default function ICSpliffsters(props) {
  ...
  const _updates = async () => {
    var stats = await api.canister("2iiif-4qaaa-aaaah-qcs5a-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const getBestPrice = bulkbuy => {
    return (bulkbuy[1] <= (BigInt(bulkbuy[0])*price) ? bulkbuy[1] : (BigInt(bulkbuy[0])*price));
  };
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      ...
      var r = await api
        .canister("2iiif-4qaaa-aaaah-qcs5a-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("2iiif-4qaaa-aaaah-qcs5a-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "top", backgroundSize: "cover",backgroundImage:"url('/collections/icspliffsters/banner.jpg')"}}></div>
        <h1>Welcome to the official ICSpliffsters sale</h1>
        </div>
        ...
        : 
          <>{(!saleOver && remaining > 0) ?
            <>
              {whitelist  ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>You are on the whitelist! You will receive a discounted mint price!</span></strong></p>
                </> : ""
              }
              {Date.now() >= startTime  ? 
                <>
                    ...
                    {buyOptions.map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        <Button
                          variant={"contained"}
                          color={"primary"}
                          onClick={() => buyFromSale(o[0], getBestPrice(o))}
                          style={{ fontWeight: "bold", margin: "0 auto" }}
                        >
                          Buy {o[0]} NFT{o[0] === 1 ? "" : "s"}<br />for {_showListingPrice(getBestPrice(o))} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
