...
export default function Frog3D(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(1000000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1645542000000);
  ...
  const _updates = async () => {
    var stats = await api.canister("xcep7-sqaaa-aaaah-qcukq-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      ...
      var r = await api
        .canister("xcep7-sqaaa-aaaah-qcukq-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("xcep7-sqaaa-aaaah-qcukq-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "top", backgroundSize: "cover",backgroundImage:"url('/collections/frog3d/banner.png')"}}></div>
        <h1>Welcome to the official 3D Assassins!!! sale</h1>
        </div>
          ...
          <>{(!saleOver && remaining > 0) ?
            <>
              {whitelist && Date.now() < startTime  ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>You are on the whitelist! The sale starts <Timestamp relative autoUpdate date={startTime/1000} />!</span></strong></p>
                </> : ""
              }
              {whitelist && Date.now() >= startTime ? 
                <>
              ...
              {!whitelist ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>Sorry, this sale is only open to whitelisted users! You can grab your own NFT from the marletplace when it opens!</span></strong></p>
                </> : ""
              }
            </>
          : 
            <p><strong><span style={{fontSize:"20px",color:"red"}}>Sorry, the sale is now over! You can grab your own NFT from the marketplace soon!</span></strong></p>
          }</>
        }
      </div>
