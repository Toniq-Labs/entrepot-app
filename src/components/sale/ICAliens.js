...
export default function ICAliens(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(55000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1642172400000);
  var totalToSell = 9213;
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("p5jg7-6aaaa-aaaah-qcolq-cai", "sale").salesStats((props.account ? props.account.address : ""));
    setPrice(stats[1]);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/icaliens/banner.jpg')"}}></div>
        <h1>Welcome to the official ICAlien sale</h1>
        </div>
