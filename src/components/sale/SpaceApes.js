...
export default function SpaceApes(props) {
  ...
  const buyFromSale = async (qty, price) => {
    ...
    var v = await props.confirm("Please confirm", "Are you sure you want to continue with this purchase of " + qty + " Ape"+(qty === 1 ? "" : "s")+" for the total price of " + _showListingPrice(price) + " ICP. All transactions are final on confirmation and can't be reversed.")
    if (!v) return;
    try {
      if (qty === 1) {
        props.loader(true, "Reserving Ape...");
      } else {
        props.loader(true, "Reserving Apes..");
      }
      const api = extjs.connect("https://boundary.ic0.app/", props.identity);
      var r = await api
        .canister("3mttv-dqaaa-aaaah-qcn6q-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("3mttv-dqaaa-aaaah-qcn6q-cai", "sale").retreive(paytoaddress);
      ...
      props.alert(
        ...
        "Your purchase was made successfully - your Ape will be sent to your address shortly"
      );
      _updates();
    } catch (e) {
      props.loader(false);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/spaceapes/banner.jpg')"}}></div>
        <h1>Welcome to the official DFINITY Space Apes sale</h1>
        </div>
                  ...
                  <Grid justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
                    {[1,2,3,4,5].map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        ...
                        >
                          Buy {o} {o === 1 ? "Ape" : "Apes"}<br />for {_showListingPrice(BigInt(o)*price)} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
