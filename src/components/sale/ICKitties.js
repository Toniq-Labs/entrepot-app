...
export default function ICKitties(props) {
  ...
  const _updates = async () => {
    var stats = await api.canister("v3zkd-syaaa-aaaah-qcm5a-cai").salesStats((props.account ? props.account.address : ""));
    setRound(stats[2][0]);
  ...
  const buyFromSale = async (qty, price) => {
    ...
    var v = await props.confirm("Please confirm", "Are you sure you want to continue with this purchase of " + qty + " Kitty"+(qty === 1 ? "" : "s")+" for the total price of " + _showListingPrice(price) + " ICP. All transactions are final on confirmation and can't be reversed.")
    if (!v) return;
    try {
      if (qty === 1) {
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("v3zkd-syaaa-aaaah-qcm5a-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/ickitties/banner.png')"}}></div>
        ...
        : 
                  ...
                  <Grid justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
                    {[1,3,5,10].map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        ...
                        >
                          Buy {o} {o === 1 ? "Kitty" : "Kitties"}<br />for {_showListingPrice(BigInt(o)*price)} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
          ...
          : 
            <p><strong><span style={{fontSize:"20px",color:"red"}}>Sorry, the sale is now over! You can grab your IC Kitty from the marketplace soon!</span></strong></p>
          }</>
        }
      </div>
