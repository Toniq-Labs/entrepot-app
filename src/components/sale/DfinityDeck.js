...
export default function DfinityDeck(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(1100000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1641826800000);
  var totalToSell = 49;
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("mk3kn-pyaaa-aaaah-qcoda-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      ...
      var r = await api
        .canister("mk3kn-pyaaa-aaaah-qcoda-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("mk3kn-pyaaa-aaaah-qcoda-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/dd/banner.png')"}}></div>
        <h1>Welcome to the official DfinityDeck:Paradigm sale</h1>
        </div>
