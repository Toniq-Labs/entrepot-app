...
export default function Tranquillity(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(700000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1638802800000);
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("6243o-giaaa-aaaah-qck5q-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      if (qty === 1) {
        props.loader(true, "Reserving Painting...");
      } else {
        props.loader(true, "Reserving Paintings..");
      }
      const api = extjs.connect("https://boundary.ic0.app/", props.identity);
      var r = await api
        .canister("6243o-giaaa-aaaah-qck5q-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("6243o-giaaa-aaaah-qck5q-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/tranquillity/avatar.jpg')"}}></div>
        <h1>Welcome to the official Tranquillity Collection sale</h1>
        </div>
        <Grid  justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
          ...
          <Grid className={classes.stat} item md={4} xs={6}>
            ...
            <span style={{fontWeight:"bold",color:"#00b894",fontSize:"2em"}}>{remaining !== false ? 54-remaining : "Loading..."}</span>
          </Grid>
        </Grid>
        <br /><br />
        {remaining === false ? 
        ...
        : 
                  ...
                  <Grid justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
                    {buyOptions.map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        ...
                        >
                          Buy {o[0]} Painting{o[0] === 1 ? "" : "s"}<br />for {_showListingPrice(o[1])} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
          ...
          : 
            <p><strong><span style={{fontSize:"20px",color:"red"}}>Sorry, the sale is now over! You can grab your Painting from the marketplace soon!</span></strong></p>
          }</>
        }
      </div>
