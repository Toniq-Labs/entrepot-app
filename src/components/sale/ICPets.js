...
export default function ICPets(props) {
  ...
  const _updates = async () => {
    var stats = await api.canister("unssi-hiaaa-aaaah-qcmya-cai").salesStats((props.account ? props.account.address : ""));
    setRound(stats[2][0]);
  ...
  const buyFromSale = async (qty, price) => {
    ...
    var v = await props.confirm("Please confirm", "Are you sure you want to continue with this purchase of " + qty + " ICPet"+(qty === 1 ? "" : "s")+" for the total price of " + _showListingPrice(price) + " ICP. All transactions are final on confirmation and can't be reversed.")
    if (!v) return;
    try {
      if (qty === 1) {
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("unssi-hiaaa-aaaah-qcmya-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/icpets/banner.png')"}}></div>
        ...
        : 
                    ...
                    {buyOptions[currentRoundNumber].map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        <Button
                          ...
                          style={{ fontWeight: "bold", margin: "0 auto" }}
                        >
                          Buy {o[0]} {o[0] === 1 ? "ICPet" : "ICPets"}<br />for {_showListingPrice(o[1])} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
          ...
          : 
            <p><strong><span style={{fontSize:"20px",color:"red"}}>Sorry, the sale is now over! You can grab your ICPet from the marketplace soon!</span></strong></p>
          }</>
        }
      </div>
