...
export default function Cyman(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(75000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1639580400000);
  const [whitelist, setWhitelist] = React.useState(false);
  
  var totalToSell = 6700;
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("xphpx-xyaaa-aaaah-qcmta-cai", "sale").salesStats((props.account ? props.account.address : ""));
    setStartTime(Number(stats[0]/1000000n));
    if (stats[1] < 75000000n) {
      setWhitelist(true);
    }
    setPrice(stats[1]);
  ...
  const buyFromSale = async (qty, price) => {
    ...
    var v = await props.confirm("Please confirm", "Are you sure you want to continue with this purchase of " + qty + " CYMAN"+(qty === 1 ? "" : "s")+" for the total price of " + _showListingPrice(price) + " ICP. All transactions are final on confirmation and can't be reversed.")
    if (!v) return;
    try {
      if (qty === 1) {
        props.loader(true, "Reserving CYMAN...");
      } else {
        props.loader(true, "Reserving CYMEN..");
      }
      const api = extjs.connect("https://boundary.ic0.app/", props.identity);
      var r = await api
        .canister("xphpx-xyaaa-aaaah-qcmta-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("xphpx-xyaaa-aaaah-qcmta-cai", "sale").retreive(paytoaddress);
      ...
      props.alert(
        ...
        "Your purchase was made successfully - your CYMAN will be sent to your address shortly"
      );
      _updates();
    } catch (e) {
      props.loader(false);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "top", backgroundSize: "cover",backgroundImage:"url('/collections/cyman/banner.jpg')"}}></div>
        <h1>Welcome to the official CYMAN sale</h1>
        <Alert style={{fontWeight:"bold",width:550, margin:"0 auto", textAlign:"center"}} severity="warning">CYMEN NFTs will be revealed after 24 hours from start of sale.</Alert><br />
        </div>
        ...
        : 
          <>{(!saleOver && remaining > 0) ?
            <>
              {whitelist  ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>You are on the whitelist! You can purchase at the whitelist price until all whitelisted allocations have been sold!</span></strong></p><br />
                </> : ""
              }
              {whitelist && Date.now() >= startTime ? 
                <>
                  <Grid justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
                    ...
                    >
                      Buy 1 CYMAN<br />for {_showListingPrice(price)} ICP
                    </Button>
                  </Grid>
                    ...
                    {buyOptions.map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        <Button
                          ...
                          style={{ fontWeight: "bold", margin: "0 auto" }}
                        >
                          Buy {o[0]} {o[0] === 1 ? "CYMAN" : "CYMEN"}<br />for {_showListingPrice(o[1])} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
              ...
              {Date.now() < startTime ? 
                <>
                  <p><strong><span style={{fontSize:"20px",color:"black"}}>The sale starts <Timestamp relative autoUpdate date={startTime/1000} />!</span></strong></p>
                </> : ""
              }
            </>
          : 
            <p><strong><span style={{fontSize:"20px",color:"red"}}>Sorry, the sale is now over! You can grab your CYMAN from the marketplace soon!</span></strong></p>
          }</>
        }
      </div>
