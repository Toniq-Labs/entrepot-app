...
export default function Vibesters(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(200000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1646143200000);
  const [whitelist, setWhitelist] = React.useState(false);
  var whitelistend = 1646146800000;
  var presaleprice = 150000000n;
  var totalToSell = 4901;
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("2l7rh-eiaaa-aaaah-qcvaa-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      ...
      var r = await api
        .canister("2l7rh-eiaaa-aaaah-qcvaa-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("2l7rh-eiaaa-aaaah-qcvaa-cai", "sale").retreive(paytoaddress);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/vibesters/banner.png')"}}></div>
        <h1>Welcome to the official Vibesters sale</h1>
        </div>
