...
export default function Memecake(props) {
  const [page, setPage] = React.useState(1);
  const [price, setPrice] = React.useState(200000000n);
  const [remaining, setRemaining] = React.useState(false);
  const [startTime, setStartTime] = React.useState(1639497600000);
  var saleOver = false;  
  const params = useParams();
  
  const _updates = async () => {
    var stats = await api.canister("txr2a-fqaaa-aaaah-qcmkq-cai", "sale").salesStats((props.account ? props.account.address : ""));
  ...
  const buyFromSale = async (qty, price) => {
    ...
    try {
      if (qty === 1) {
        props.loader(true, "Reserving DICK...");
      } else {
        props.loader(true, "Reserving DICKs..");
      }
      const api = extjs.connect("https://boundary.ic0.app/", props.identity);
      var r = await api
        .canister("txr2a-fqaaa-aaaah-qcmkq-cai", "sale")
        .reserve(
          price,
          qty,
      ...
      while (true) {
        try {
          ...
          r3 = await api.canister("txr2a-fqaaa-aaaah-qcmkq-cai", "sale").retreive(paytoaddress);
      ...
      props.alert(
        ...
        "Your purchase was made successfully - your DICK will be sent to your address shortly"
      );
      _updates();
    } catch (e) {
      props.loader(false);
  ...
  return (
    <>
      <div style={styles.main}>
        ...
        <div style={{width: "100%", height: 200, borderRadius:10,backgroundPosition: "center", backgroundSize: "cover",backgroundImage:"url('/collections/mc/banner.jpg')"}}></div>
        <h1>Welcome to the official MemeCake sale</h1>
        </div>
        <Grid  justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
          ...
          <Grid className={classes.stat} item md={4} xs={6}>
            ...
            <span style={{fontWeight:"bold",color:"#00b894",fontSize:"2em"}}>{remaining !== false ? toSell-remaining : "Loading..."}</span>
          </Grid>
        </Grid>
        <br /><br />
        {remaining === false ? 
                  ...
                  <Grid justifyContent="center" direction="row" alignItems="center" container spacing={2} style={{}}>
                    {buyOptions.map(o => {
                      return (<Grid className={classes.stat} item sm={3}>
                        ...
                        >
                          Buy {o[0]} DICK{o[0] === 1 ? "" : "s"}<br />for {_showListingPrice(o[1])} ICP
                        </Button>
                      </Grid>);
                    })}
                  </Grid>
