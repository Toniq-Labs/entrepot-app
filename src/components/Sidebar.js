...
export default function Wallet(props) {
  ...
  const refresh = async () => {
    if (props.account) {
      ...
      setBalance(b);
      var volt = await api.canister("flvm3-zaaaa-aaaak-qazaq-cai").hasOwnerCanister(props.identity.getPrincipal());
      ...
      for (var i = 0; i < props.collections.length; i++) {
        ...
        try {
          ...
          if (collection.canister === 'bxdf4-baaaa-aaaah-qaruq-cai') {
            ...
            tokens = tokens.concat(
              await api
                ...
                .getTokens(props.account.address, props.identity.getPrincipal().toText()),
            );
          } else if (collection.canister === 'y3b7h-siaaa-aaaah-qcnwa-cai') {
            tokens = tokens.map(a => {
              a.wrapped = true;
              return a;
            });
          ...
          } else if (collection.canister === '3db6u-aiaaa-aaaah-qbjbq-cai') {
            tokens = tokens.map(a => {
              a.wrapped = true;
              return a;
            });
          ...
          } else if (collection.canister === 'q6hjz-kyaaa-aaaah-qcama-cai') {
            tokens = tokens.map(a => {
              a.wrapped = true;
              return a;
            });
          ...
          } else if (collection.canister === 'jeghr-iaaaa-aaaah-qco7q-cai') {
            tokens = tokens.map(a => {
              a.wrapped = true;
              return a;
            });
      ...
      {props.account !== false ? (
        <List>
          ...
          {props.accounts.length > 1 ? (
            <ListItem>
              <Button
                onClick={e => setAnchorElLogin(e.currentTarget)}
                fullWidth
                variant="outlined"
                color="primary"
          ...
          <ListItem>
            ...
            >
              ...
              <MenuItem onClick={() => login('infinitywallet')}>
                <ListItemIcon>
                  <img alt="I" src="/plug.png" style={{height: 26}} />
                </ListItemIcon>
                ...
                <ListItemSecondaryAction>
                  <ListItemIcon>
                    <Button
                      color={'primary'}
                      variant={'contained'}
                      onClick={() => navigate('/marketplace/' + props.collection.route)}
                      style={{marginTop: '3px', marginLeft: '30px'}}
                      size="small"
                      edge="end"
                    >
                      Back
                ...
                {myCollections.length === 0 ? (
                  ...
                  <>
                    {myCollections.map(_collection => {
                      return (
                        <ListItem
                          key={_collection.canister + '-' + _collection.count}
                          selected={
                            props.view === 'wallet' && _collection.route == props.collection?.route
                          }
                          button
                          onClick={() => navigate('/wallet/' + _collection.route)}
                        >
                          <ListItemAvatar>
                            <Avatar>
                              <img
                                alt={_collection.name}
                                src={'/collections/' + _collection.canister + '.jpg'}
                                style={{height: 64}}
                              />
                            </Avatar>
                          </ListItemAvatar>
                          ...
                          <ListItemSecondaryAction>
                            <Chip label={_collection.count} variant="outlined" />
                          </ListItemSecondaryAction>
                        </ListItem>
                      );
                    })}
