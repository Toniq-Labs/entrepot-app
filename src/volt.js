...
export async function loadVolt(identity) {
  ...
  const voltFactoryAPI = extjs.connect('https://ic0.app/', identity).canister('flvm3-zaaaa-aaaak-qazaq-cai');
...
export async function loadVoltBalance(identity) {
  ...
  if (loadedVolts.hasOwnProperty(currentPrincipal)){
    ...
    const voltAPI = extjs.connect('https://ic0.app/', identity).canister(voltPrincipal, 'volt');
    const resp = await voltAPI.getBalances('icpledger', 'ryjl3-tyaaa-aaaaa-aaaba-cai', []);
    if (resp.hasOwnProperty('ok')) {
      return [Number(resp.ok[0]), Number(resp.ok[1]), Number(resp.ok[2])];
    } else return false;
  };
  return false;
}
