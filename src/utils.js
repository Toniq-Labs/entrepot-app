...
EntrepotEarnDetails = (id,nft_price) => {
  ...
  if (earnData.hasOwnProperty(id)) {
    ...
    return (<div style={{padding:"5px 0",fontSize:11, fontWeight:"bold", textAlign:"left", borderTop:"1px solid #ddd"}}>
      Receive NFT or Profit <PriceICP style={(nft_price < (earnData[id].reward+earnData[id].amount) ? {color:"green"} : {color:"red"})} price={nft_price - (earnData[id].reward+earnData[id].amount)} /><br /><Timestamp relative autoUpdate date={Number((earnData[id].filled[0]+earnData[id].length) / 1000000000n)}/>
    </div>);
  };
  return "";
},
