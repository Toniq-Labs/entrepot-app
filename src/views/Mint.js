      ...
      if (false) {
        ...
        videoPlayer.addEventListener('loadedmetadata', () => {
          ...
          videoPlayer.addEventListener('seeked', () => {
            var scaleRatio =
              Math.min(...boundBox) / Math.max(videoPlayer.videoWidth, videoPlayer.videoWidth);
...
export default function Mint(props) {
  ...
  const transferaction = async () => {
    ...
    var v = await props.confirm(
      'Are you sure?',
      'Please ensure your have selected the right file and you clicked the right advanced action - Bulk Transfer',
    );
    if (!v) return;
  ...
  const listaction = async () => {
    ...
    var v = await props.confirm(
      'Are you sure?',
      'Please ensure your have selected the right file and you clicked the right advanced action - Bulk List',
    );
    if (!v) return;
  ...
  const mintaction = async () => {
    ...
    for (var i = 0; i < selectedFiles.length; i++) {
      ...
      var thumb = await generateThumbnail(
        selectedFiles[i],
        [
          300,
          300,
    ...
    props.loader(false);
    props.alert('Minting complete', 'Your NFTs have been minted and sent to you');
  };
          ...
          <Grid container spacing={2}>
            <Grid style={{textAlign: 'center'}} item xs={12} sm={12} md={12}>
              <div>
                ...
                {canister ? (
                  <>
                    <input type="file" onChange={selectFiles} multiple />
                    <br />
                    <br />
                    <Button
                      onClick={mintaction}
                      variant="contained"
                      color="primary"
                      style={{backgroundColor: '#003240', color: 'white', marginRight: 10}}
                    >
                      Mint as NFTs
                    </Button>
                    <Button
                    ...
                    <Button
                      onClick={() => clipboardCopy(canister)}
                      target="_blank"
                      variant="contained"
                      color="primary"
                    ...
                    <input type="file" onChange={selectFiles2} />
                    <br />
                    <br />
                    <Button
