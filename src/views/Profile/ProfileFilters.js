    ...
    <>
      ...
      {Object.keys(props.nftFilterStats.traits).length > 0 && (
        <div>
          ...
          >
            ...
            <ToniqIcon
              icon={
                isSectionExpanded[topLevelTraitsExpandKey] ? ChevronUp24Icon : ChevronDown24Icon
              }
            ></ToniqIcon>
          </div>
          <div style={{display: isSectionExpanded[topLevelTraitsExpandKey] ? 'block' : 'none'}}>
            <div
              style={{
            ...
            <div>
              {Object.keys(props.nftFilterStats.traits)
                ...
                .map(collectionId => {
                  const collection = props.nftFilterStats.traits[collectionId].collection;
                  const traits = props.nftFilterStats.traits[collectionId].traitsInUse;
                  const collectionExpandKey = collectionTraitsExpandKey(collection.id);
                  ...
                  return (
                    <>
                      ...
                      <div
                        style={{
                          display: isSectionExpanded[collectionExpandKey] ? 'block' : 'none',
                        }}
                      >
                        {Object.keys(traits).map(traitId => {
                          const traitExpandKey = collectionIndividualTraitExpandKey(
                            collectionId,
                            traitId,
                          );
                          const traitName = collection.traits[0][traitId][1];

                          const isTraitSelected = Object.values(
                            props.filters.traits[collectionId]?.[traitId] ?? [],
                          ).some(value => !!value);
                            ...
                            <div key={traitExpandKey}>
                              ...
                              >
                                ...
                                <div style={{display: 'flex', gap: '4px'}}>
                                  <ToniqIcon
                                    icon={
                                      isSectionExpanded[traitExpandKey]
                                        ? ChevronUp24Icon
                                        : ChevronDown24Icon
                                    }
                                  ></ToniqIcon>
                              ...
                              <div
                                style={{
                                  display: isSectionExpanded[traitExpandKey] ? 'block' : 'none',
                                }}
                              >
                                {Object.keys(traits[traitId]).map(traitStyleId => {
                                  const traitStyleName =
                                    collection.traits[0][traitId][2][traitStyleId][1];

                                  const isTraitTypeSelected =
                                    props.filters.traits[collectionId]?.[traitId]?.[traitStyleId];
                                  return (
                                    <div
                                      style={{
                                        ...(isTraitTypeSelected
                                        ...
                                        props.updateFilters({
                                          ...
                                          traits: {
                                            ...
                                            [collectionId]: {
                                              ...
                                              [traitId]: {
                                                ...props.filters.traits[collectionId]?.[traitId],
                                                [traitStyleId]: !isTraitTypeSelected,
                                              },
                                            },
                                          },
                                        });
                                    ...
                                    >
                                      <span>{traitStyleName}</span>
                                      {makeCountIndicator(
                                        traits[traitId][traitStyleId],
                                        isTraitTypeSelected,
                                      )}
                                    </div>
                                  );
      ...
      {props.userCollections.length > 1 && (
        <div>
          ...
          <div
            style={{display: isSectionExpanded[topLevelCollectionsExpandKey] ? 'block' : 'none'}}
          >
            <div
              style={{
                ...
                .map(collection => {
                  return (
                    <SelectableNameWithCount
                      ...
                      onClick={() => {
                        const isAlreadyIncluded = props.filters.collections.includes(
                          collection.name,
                        );
